    ...
    const rows = installList?.values.map((e, i) => (
        <Tooltip label="View Detail" withArrow key={e.id} openDelay={1000}>
            <Table.Tr onClick={() => navigate(`/install/${e.id}`)}>
                ...
                <Table.Td c={e?.shop?.name ? undefined : "#bbbbbb"}>{e?.shop?.name ?? "No Data"}</Table.Td>
                <Table.Td c={e?.edgeBox?.name ? undefined : "#bbbbbb"}>{e?.edgeBox?.name ?? "No Data"}</Table.Td>
                <Table.Td c={e?.createdDate ? undefined : "#bbbbbb"}>{e?.createdDate ? removeTime(new Date(e?.createdDate), "/") : "No Data"}</Table.Td>
                <Table.Td c={e?.uninstalledTime ? undefined : "#bbbbbb"}>{e?.uninstalledTime ? removeTime(new Date(e?.uninstalledTime), "/") : "No Data"}</Table.Td>
                <Table.Td ta={"center"}>
